...
class Tester(object):
    ...
    def infer(self):
        ...
        for i in range(0, self.data_length):
            print(i)
            modal, category, bboxes, amodal_gt, image_fn, occ_l = self.data_reader.get_image_instances(
            ...
            if self.args.amodal_method == 'aw_sdm5': # supervised
                amodal_patches_pred = infer.infer_amodal_aw_sdm(
                    self.model, image_fn, modal, category, bboxes,
                    use_rgb=self.args.model['use_rgb'], th=amodal_th, input_size=512,
                    min_input_size=16, interp='nearest', args=args)
        ...
        print("Evaluation results.  \
              mIoU: {:.5g}, pAcc: {:.5g}, inv_mIoU: {:.5g}".format(miou, pacc, inv_miou))
